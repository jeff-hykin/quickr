...
import { FileSystem } from "https://deno.land/x/quickr@0.3.9/main/file_system.js"
import { Console, bold, lightRed, yellow } from "https://deno.land/x/quickr@0.3.9/main/console.js"
import { run, Timeout, Env, Cwd, Stdin, Stdout, Stderr, Out, Overwrite, AppendTo, throwIfFails, returnAsString, zipInto, mergeInto } from "https://deno.land/x/quickr@0.3.9/main/run.js"
// import { FileSystem } from "/Users/jeffhykin/repos/quickr/main/file_system.js"
// import { Console, bold, lightRed, yellow } from "/Users/jeffhykin/repos/quickr/main/console.js"
...
for (const each of await FileSystem.recursivelyListItemsIn(testsFolder)) {
    if (each.isFile) {
        ...
        try {
            var { success } = await run(each.path, Out(Overwrite(logPath)))
        } catch (error) {
            console.log(bold.lightRed`    Error with test: `.yellow`${relativePart}: ${error.message}`)
            continue
        }
        if (!success) {
            console.log(bold.lightRed`    Error with test: `.yellow`${relativePart}: ${logPath}`)
        } else {
            console.log(bold.lightGreen`    Passed: `.yellow`${relativePart}: ${logPath}`)
        }
    }
}