...
import { Buffer } from "https://deno.land/std@0.162.0/io/buffer.ts"
...
export function writableStreamFromWriter(writer, options={},) {
    const { autoClose = true, ignoreErrors = false } = options;
 ...
 * ```
 */
export async function* iterateReader(r, options={bufSize: DEFAULT_BUFFER_SIZE},) {
 ...
 */
export function* iterateReaderSync(r, options={ bufSize: DEFAULT_BUFFER_SIZE}, ) {
    const bufSize = options?.bufSize ?? DEFAULT_BUFFER_SIZE;
    const b = new Uint8Array(bufSize);
    while (true) {
 ...
 */
export async function copy(src, dst, options={ bufSize: DEFAULT_BUFFER_SIZE }, ) {
    let n = 0;
    const bufSize = options?.bufSize ?? DEFAULT_BUFFER_SIZE;
    const b = new Uint8Array(bufSize);
    let gotEOF = false;
