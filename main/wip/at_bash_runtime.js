...
// 
    ...
    export const helpers = {
        ...
        glob({ globString, shellEngine }) {
            // FIXME: handle the ? 
            // FIXME: handle the []'s
            // FIXME: check glob options (e.g. including "." and ".." and hidden or not)
            let names = [...Deno.readDirSync(shellEngine.internalInfo.pwd)].map(each=>each.name)
        ...
        getExecutableName({ children, shellEngine }) {
        ...
        getArgument({ argumentParseData, shellEngine }) {
    ...
    export class Statement {
        constructor({type, parseData, shellEngine}) {
            this.type = type
            ...
            if (this.type == "command") {
                const env = this.shellEngine.createCommandEnv()
                const executableName = helpers.getExecutableName({ children: this.parseData.children[0].children, shellEngine: this.shellEngine })
                const argumentParseData = this.parseData.children.slice(1)
                // FIXME might need to ignore line-escapes here
                const commandArgs = argumentParseData.map(each=>helpers.getArgument({argumentParseData: each, shellEngine: this.shellEngine}))
                ...
                this.customData.conditionalStatement = new Statement({ type: "conditional", parseData: children[1], shellEngine })
                ...
                new Statement({ type: "statement", parseData: children[2], shellEngine })
            } else {
        ...
        async _createCommand({commandName, options, afterCommand, afterProcess, afterComplete}) {
            ...
            this.shellEngine.previousPid = childProcess.pid
            ...
            this.shellEngine.prevousExitCode = code
    ...
    import { Parser, parserFromWasm, flatNodeList } from "https://deno.land/x/deno_tree_sitter@0.2.1.3/main.js"
...
// 
    ...
    const s1 = new Statement({
        shellEngine,
        type: "command",
        parseData: parser.parse(`echo 'hi'`).rootNode.children[0],
        // parseData: {
        //     type: "command",
        //     children: [
        //         {
