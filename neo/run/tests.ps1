...
import $ from "https://esm.sh/gh/jeff-hykin/dax@88bc256/mod.ts"
const $$ = (...args)=>$(...args).noThrow()
// await $$`false`
// await $$`false`.text("stderr")
const argsWereGiven = Deno.args.length > 0

const testsFolder = `${FileSystem.thisFolder}/../main`
...
for (const each of await FileSystem.recursivelyListItemsIn(testsFolder)) {
    if (each.isFile) {
        ...
        if (argsWereGiven && !Deno.args.some(eachMentioned=>each.name.includes(eachMentioned))) {
            continue
        }
        ...
        if (each.path.endsWith(".js")) {
            ...
            const logFile = $.path(`${each.path}.log`)
            var { code } = (
                // await $$`deno --version`
                await $$`deno test --doc --no-lock -A ${each.path} 2>&1`.env({
                    NO_COLOR:"true"
                }).stdout(
                    logFile
                )
            ...
            if (!success) {
                console.log(bold.lightRed`    Error with test: `.yellow`${each.name}:`.blue` ${relativePart}.log`)
            } else {
                console.log(bold.lightGreen`    Passed: `.yellow`${each.name}:`.blue` ${relativePart}.log`)
            }
        }
    }
}
